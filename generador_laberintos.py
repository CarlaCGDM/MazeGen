...
#############################################################################
#             FUNCIÓN QUE CONVIERTE LOS VALORES DE UN LABERINTO             #
#            (Caminos = 1, Paredes = 0, Entrada = 2, Salida = 3)            #
#############################################################################

def formatear_laberinto(laberinto,pared,camino,entrada,salida):
    valores_convertidos = {'p': pared,'c': camino,'e': entrada,'s': salida}
    alto = len(laberinto)
    ancho = len(laberinto[0])
    for i in range(alto):
        for j in range(ancho):
            valor = laberinto[i][j]
            laberinto[i][j] = valores_convertidos[valor]
    return laberinto
    

if __name__ == '__main__':
    laberinto = generar_laberinto(40,10)
    exportar_laberinto(laberinto,'salida.csv')
    print(formatear_laberinto(laberinto,0,1,2,3))
