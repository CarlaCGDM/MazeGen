...
def generar_laberinto(min=10,max=15):
    ...
    alto = random.randint(10,15)
    ancho = random.randint(10,15)

    #Generamos un laberinto y lo exportamos:
    laberinto_generado = generador.generar_laberinto(ancho,alto)
    generador.exportar_laberinto(laberinto_generado,'salida.csv')

#-------------------------------------------------------------------------------------------#
#                                   RESOLVER EL LABERINTO                                   #
#-------------------------------------------------------------------------------------------#

def cargar_laberinto(file):
    #Accedemos al fichero del laberinto:
    fichero = 'salida.csv'
    raiz = Path(file).parents[1]
    ruta_fichero = raiz / fichero

    #Cargamos la matriz del laberinto como array de numpy:
    matriz = numpy.loadtxt(ruta_fichero, delimiter=",", dtype=int)
    return matriz
